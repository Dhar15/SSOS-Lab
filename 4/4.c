/* Question - Design, develop and implement YACC/C program to demonstrate Shift Reduce Parsing technique 
   for the grammar rules: E -> E+T | T, T -> T*F | F, F -> (E) | id and parse the sentence: id + id * id. */

#include<stdio.h>
#include<string.h>
int i=0, j=0, k=0, len=0;
char input[16], reduce[20], stack[15], shift[10], symbol[2];

void check() {
	strcpy(reduce, "REDUCE TO ");
	for(k=0; k<len; k++)
		if(stack[k]=='i' && stack[k+1]=='d') {
			stack[k]='F';
			stack[k+1]='\0';
			printf("\n$%s\t%s$\t%s%c", stack, input, reduce, stack[k]);
			j++;
		}
...
int main() {
	...
	for(i=0, j=0; j<len; i++,j++) {
		if(input[j]=='i' && input[j+1]=='d') { // id
			...
			symbol[0] = input[j];
			input[j]=' ';
			printf("\n$%s\t%s$\t%s%s", stack, input, shift,symbol);
			check();
		}
	}
